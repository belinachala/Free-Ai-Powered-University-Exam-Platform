...
// Academic ranks
const ranks = ["Lecturer", "Assistant Professor", "Associate Professor", "Professor"];
...
const UniversityTeachersProfileCompletion: React.FC = () => {
  ...
  useEffect(() => {
    if (verified) {
      setTimeout(() => navigate("/uni-teacher"), 800);
    }
  }, [verified, navigate]);
  ...
  const validateForm = () => {
    for (let key in profile) {
      if (!profile[key as keyof TeacherProfile] && key !== "profile_picture" && key !== "profile_picture_preview") {
        ...
        >
          ...
          {!verificationSent && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="form-label font-semibold">Course(s) Taught</label>
                  <input
                    type="text"
                    name="courses_taught"
                    value={profile.courses_taught}
                    onChange={handleChange}
                    className="form-control border-2 border-blue-400"
                  />
                </div>
                <div className="flex-1">
                  <label className="form-label font-semibold">Academic Rank</label>
                  <select
                    name="academic_rank"
...
export default UniversityTeachersProfileCompletion;
